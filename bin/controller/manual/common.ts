...
import { formatManualDirContentBeforeStore, loadDirData } from "../../utils";
...
const migrationExec = async (op:string, args: CommandlineArgs) => {
  ...
  const formattedResultBeforeStore = await formatManualDirContentBeforeStore(data, dataLocationDir);
